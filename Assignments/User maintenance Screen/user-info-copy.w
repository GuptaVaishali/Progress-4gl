...
DEFINE FRAME DEFAULT-FRAME
     ...
     UpdateBtn AT ROW 25.76 COL 151 WIDGET-ID 52
     BtnExit AT ROW 25.76 COL 171 WIDGET-ID 32
...
/* SETTINGS FOR BUTTON UpdateBtn IN FRAME DEFAULT-FRAME
   NO-ENABLE                                                            */
...
DO:
    ...
    IF (btnSave:SENSITIVE = TRUE AND btnCancel:SENSITIVE = TRUE) OR updatebtn:SENSITIVE = TRUE THEN
    DO:
...
DO:   
    ...
    DO:
            ...
            Assign TTNewUser.createdDateTime = NOW.   
            Assign TTNewUser.lastUpdatedDateTime = NOW.        
...
&Scoped-define SELF-NAME UpdateBtn
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL UpdateBtn C-Win
ON CHOOSE OF UpdateBtn IN FRAME DEFAULT-FRAME /* Update */
DO:
  DEF VAR valid AS LOGICAL NO-UNDO INITIAL TRUE.
    RUN checkValidation(INPUT-OUTPUT valid).
    
    /*******Saving record in temp table and db**********/
    IF valid AND NOT isAdd THEN
    DO:
            FIND FIRST newUser WHERE newuser.user_id = FUserId:SCREEN-VALUE EXCLUSIVE-LOCK NO-WAIT NO-ERROR.
            IF NOT AVAILABLE newUser THEN
                 IF LOCKED newUser THEN
...
DEFINE INPUT PARAMETER puser_id LIKE NewUser.user_id.
DEFINE INPUT PARAMETER puser_group LIKE NewUser.USERGROUP.
